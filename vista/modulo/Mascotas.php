<?php 
	$arrayMascotas = ControladorMascota::seleccionarMascotasCtl();
	...
	$paginacion = Paginacion::pnt($modulo, sizeof($arrayMascotas), $init, $size);
	$paginacion != null ? $inicio = $paginacion['inicio'] : null ;
	$mascotas = array_slice($arrayMascotas, $inicio, $size);
?>

<div class="title">
	<h2>Mascotas</h2>
	<a class="link-button active" href="index.php?pagina=Usuarios">Mascotas</a>
...
<div class="C__Table">
	...
	<?php }else{ ?>
		...
		<div class="Bar__Btns">
			...
			<div class="C__Btn">
				<input type="image" src="img/treatment_32px.png" alt="imágen de acción" id="btn-see-HC-pet" onclick="historiaClinica()" disabled>
		...
		<!-- <div class="nodata"><span>Ajustar formulario de Consulta</span></div> -->
		...
		<div class="C__f oculto" id="form-edit-pet">
			<form method="post" class="f">
				...
				<div class="i__group flex">
					<div class="i__group">
						<label class="labels" for="pet-property-edit">Selecciona Propietario</label>
						<select name="pet-property-edit" id="pet-property-edit">
						<?php foreach ($clientes AS $k => $v) : ?>
							<option value="<?=$v["iduser"]?>"><?=$v["nombre"]?></option>
						<?php endforeach; ?>
						</select>
					</div>
					<div style="align-self: center">ó</div>
					<div class="C__Btn">
						<a class="submit" href="index.php?pagina=Clientes&openForm=form-add-client">Agregar Cliente</a>
					</div>
				</div>
				<div class="i__group">
	...
	<div class="C__f oculto" id="form-add-pet">
		<form method="post" class="f">
			...
			<div class="i__group flex">
				<div class="i__group">
					<label class="labels" for="pet-property-new">Selecciona Propietario</label>
					<select name="pet-property-new" id="pet-property-new">
					<?php foreach ($clientes AS $k => $v) : ?>
						<option value="<?=$v["iduser"]?>"><?=$v["nombre"]?></option>
					<?php endforeach; ?>
					</select>
