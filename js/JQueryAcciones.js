export default class JQueryAcciones {
	...
	static addElementsChoosen (elements, form) {
	...
	static editForm (btnShowFormEdit, checkElement, inputId, callback) {
		if (btnShowFormEdit) {
			$(btnShowFormEdit).click(function (){
				let recordId = JQueryAcciones.elementChecked(checkElement);
				inputId.val(recordId);
	...
	static deleteForm (btnShowFormDelete, btnConfirmDelete, btnCloseForm, checkElements, form, module, view) {
		if (btnShowFormDelete && btnCloseForm && btnConfirmDelete && form) {
			$(btnShowFormDelete).click(function () {
				JQueryAcciones.addElementsChoosen(checkElements, form);
			});
			...
			$(btnCloseForm).click(function () {
				JQueryAcciones.removeElementsChoosen();
			});
		}
	}
	
	static search(input, table, moduleSearch, callback) {
		if (input && table) {
			input.keyup(() => {
	...
	static completarHoraInicio(fecha){
		...
		return ano+"-"+JQueryAcciones.minTwoDigits(mes)+"-"+JQueryAcciones.minTwoDigits(dia)+"T"+JQueryAcciones.minTwoDigits(hora)+":"+JQueryAcciones.minTwoDigits(minutos);
	}
